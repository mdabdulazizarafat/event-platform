# Ayojok Event Platform — Master Technical Documentation & Scalability Specification
...
**Ayojok** (`ayojok.rongplan.com`) is a high-performance, enterprise-grade Event Management, Ticketing, and Attendance Verification Platform tailored for large-scale conferences, tech expos, hackathons, and multi-track events. 
...
1. **Discovery & Browsing**: User visits `ayojok.rongplan.com`, browses active events (`GET /api/v1/events`).
...
├── nginx.ayojok.conf        # Production Host Nginx Reverse Proxy Configuration
...
Configure `/etc/nginx/sites-available/ayojok` with worker connection tuning, keepalive pooling, and compression:

```nginx
# Upstream Keepalive Connection Pool
upstream ayojok_backend {
...
server {
    ...
    location /api/ {
        proxy_pass http://ayojok_backend;
