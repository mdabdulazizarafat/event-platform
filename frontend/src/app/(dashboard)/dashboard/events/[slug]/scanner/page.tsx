...
export default function QRScannerPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug: queryEventSlug } = React.use(params);
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6 w-full mx-auto">
      <PageHeader 
        title="Check-In Door Scanner" 
        description="Offline-resilient scanning dashboard for physical event check-ins."
        action={
